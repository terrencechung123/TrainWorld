...
class TestUser:
    ...
    def test_has_list_of_trains(self):
        '''has records with lists of trains records attached.'''
        ...
        with app.app_context():
            ...
            user.trains.append(train_1)
            user.trains.append(train_2)

            db.session.add_all([user, train_1, train_2])
            db.session.commit()
            ...
            # check that trains were saved to user
            assert(train_1 in user.trains)
            assert(train_2 in user.trains)
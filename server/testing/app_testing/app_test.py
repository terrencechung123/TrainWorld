...
class TestTrainIndex:
    '''TrainIndex resource in app.py'''

    def test_lists_trains_with_200(self):
        '''returns a list of trains associated with the logged in user and a 200 status code.'''
    ...
    def test_creates_trains_with_201(self):
        '''returns a list of trains associated with the logged in user and a 200 status code.'''
        ...
        with app.test_client() as client:
            ...
            with client.session_transaction() as session:
                ...
                new_train = Train.query.filter(Train.user_id == session['user_id']).first()

            assert response_json['title'] == new_train.title
            assert response_json['instructions'] == new_train.instructions
            assert response_json['minutes_to_complete'] == new_train.minutes_to_complete

    def test_returns_422_for_invalid_trains(self):
        with app.app_context():
